...
const App: React.FC = () => {
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      const newImages: UserImage[] = (Array.from(files) as File[]).map((file, index) => ({
        id: `upload-${Date.now()}-${index}-${Math.random().toString(36).substr(2, 5)}`,
  ...
  const clearGallery = () => {
    if (window.confirm("Xóa toàn bộ lịch sử ảnh trong phiên này?")) setResults([]);
  };
  ...
  const generateSingleImage = async (image: UserImage, style: StyleType, ratio: AspectRatio, quality: Quality) => {
    ...
    try {
      ...
      const errMsg = err.message === 'quota' 
        ? "Máy chủ đang quá tải (429). Vui lòng đợi 15 giây để tiếp tục." 
        : (err.message || "Lỗi kết xuất ảnh.");
      setError(errMsg);
      return false;
    }
  };
  ...
  const handleGenerateAll = async () => {
    ...
    for (let i = 0; i < userImages.length; i++) {
      ...
      if (i < userImages.length - 1) await new Promise(res => setTimeout(res, 1500));
    }
  ...
  const downloadImage = (url: string) => {
    ...
    link.href = url;
    link.download = `LoiAI_Portrait_${Date.now()}.jpg`;
  ...
  const handleDownloadAll = () => {
    results.filter(r => r.status === 'completed').reverse().forEach((res, idx) => {
      setTimeout(() => downloadImage(res.url), idx * 800);
    });
  };
  ...
  return (
    <div className="min-h-screen flex flex-col selection:bg-blue-500/30">
          ...
          <div className="bg-[#0c162d]/60 backdrop-blur-3xl border border-blue-500/10 rounded-[3rem] p-8 shadow-2xl space-y-8 ring-1 ring-blue-400/5">
            ...
            <section className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                {userImages.length > 0 && (
                  <button onClick={clearAllUploads} className="text-[10px] font-bold text-red-400 uppercase tracking-widest hover:opacity-70 transition-opacity">Xóa nguồn</button>
                )}
              </div>
              <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-blue-500/20 rounded-[2.5rem] cursor-pointer bg-blue-500/5 hover:bg-blue-500/10 hover:border-blue-400/40 transition-all group relative overflow-hidden">
                <div className="flex flex-col items-center justify-center p-6 text-center z-10">
                  ...
                  <p className="text-sm font-bold text-white opacity-80">Chọn ảnh gốc</p>
                  <p className="text-[9px] text-blue-400/60 mt-1 uppercase tracking-widest font-black italic">Identity Locking Enabled</p>
                </div>
            ...
            <section className="space-y-4">
              ...
              <div className="flex gap-2">
                 {Object.values(AspectRatio).map(r => (
                   <button key={r} onClick={() => setSelectedRatio(r)} className={`flex-1 py-3.5 rounded-2xl border text-[10px] font-black transition-all ${selectedRatio === r ? 'bg-white text-[#050a1a] border-white shadow-xl scale-[1.05]' : 'bg-blue-900/20 border-blue-500/5 text-blue-400 hover:text-blue-100'}`}>{r}</button>
                 ))}
              </div>
            ...
            >
              {isGenerating ? 'ĐANG BIÊN TẬP...' : `BẮT ĐẦU XUẤT BẢN (${userImages.length})`}
            </button>
            
            {error && (
              <div className="p-5 rounded-[2rem] bg-red-950/40 border border-red-500/20 text-red-200 animate-in slide-in-from-bottom-2">
                <p className="text-[10px] font-black uppercase tracking-widest leading-relaxed text-center">{error}</p>
                <button onClick={() => setError(null)} className="w-full mt-3 text-[9px] font-bold text-red-400/60 uppercase tracking-widest hover:text-red-300">Đóng thông báo</button>
              </div>
            )}
          </div>
        </aside>
        ...
        <section className="flex-grow flex flex-col gap-8 h-full min-w-0">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
             <div className="space-y-1">
               <h2 className="text-5xl font-black text-white tracking-tighter uppercase leading-none italic">Studio Gallery</h2>
               <p className="text-blue-400 text-[10px] font-black uppercase tracking-[0.4em] opacity-60">Full-Frame Mode • Precise Identity Locking</p>
             </div>
             <div className="flex gap-3">
               {results.length > 0 && (
                 <button onClick={clearGallery} className="bg-red-500/5 hover:bg-red-500/10 text-red-400/80 border border-red-500/10 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all">Dọn dẹp</button>
               )}
               {results.some(r => r.status === 'completed') && (
                 <button onClick={handleDownloadAll} className="bg-blue-500/10 hover:bg-blue-500/20 text-blue-100 border border-blue-400/20 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-2xl backdrop-blur-md">Tải toàn bộ</button>
               )}
             </div>
          </div>

          <div className="flex-grow overflow-y-auto pr-2 scrollbar-hide pb-40">
            {results.length === 0 && !isGenerating ? (
              <div className="h-full min-h-[550px] flex flex-col items-center justify-center border-2 border-dashed border-blue-500/10 rounded-[4rem] text-blue-900/20 bg-blue-500/[0.01]">
                ...
                <p className="text-[9px] uppercase tracking-widest mt-2 opacity-40 italic">Đang chờ dữ liệu chân dung từ bạn</p>
              </div>
            ) : (
                    ...
                    <div className={`relative ${res.ratio === AspectRatio.PORTRAIT ? 'aspect-[9/16]' : (res.ratio === AspectRatio.LANDSCAPE ? 'aspect-[16/9]' : 'aspect-square')} bg-[#050a1a]`}>
                      {res.status === 'pending' ? (
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center p-10 text-center bg-red-950/10">
                           ...
                           <p className="text-[10px] font-black text-red-300 uppercase tracking-widest leading-relaxed">Kết xuất không thành công<br/><span className="opacity-50 text-[8px]">Hệ thống sẽ tự phục hồi sau vài giây</span></p>
                        </div>
                      ) : (
                        <>
                          <img src={res.url} className="w-full h-full object-cover transition-transform duration-[8s] group-hover:scale-105" alt="AI Generated" />
                          <div className="absolute inset-0 bg-gradient-to-t from-[#050a1a] via-[#050a1a]/10 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-700 flex flex-col justify-end p-12">
                            <div className="flex gap-4 translate-y-10 group-hover:translate-y-0 transition-transform duration-700 ease-out">
                              <button onClick={() => setSelectedViewImage(res.url)} className="flex-1 bg-white text-[#050a1a] py-5 rounded-[1.25rem] font-black text-[11px] uppercase tracking-widest shadow-2xl transition-all hover:bg-blue-50 active:scale-95">Xem chi tiết</button>
                    ...
                    <div className="p-10 space-y-4">
                      <div className="flex justify-between items-center">
                        <div className="flex flex-col">
                           <span className="text-[13px] font-black text-white uppercase tracking-widest">MASTERPIECE {(results.length - idx).toString().padStart(2, '0')}</span>
                           <span className="text-[8px] text-blue-500 font-black uppercase tracking-[0.2em] mt-1">Loi AI Studio • {res.style.split(':')[0]}</span>
                        </div>
      ...
      {/* FULLSCREEN MODAL */}
      {selectedViewImage && (
        <div className="fixed inset-0 z-[100] bg-[#050a1a]/98 flex items-center justify-center p-4 md:p-12 cursor-zoom-out backdrop-blur-3xl animate-in fade-in duration-500" onClick={() => setSelectedViewImage(null)}>
          <div className="relative group max-w-full max-h-full animate-in zoom-in-95 duration-500" onClick={(e) => e.stopPropagation()}>
            <img src={selectedViewImage} className="max-w-full max-h-[92vh] rounded-[3rem] shadow-[0_0_120px_rgba(37,99,235,0.25)] border border-white/10 object-contain ring-1 ring-white/10" alt="Full" />
            <div className="absolute top-8 right-8 flex gap-3">
              <button 
                onClick={(e) => { e.stopPropagation(); downloadImage(selectedViewImage); }} 
                className="w-16 h-16 rounded-[1.5rem] bg-white text-[#050a1a] flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all"
              >
                <Icons.Download />
              </button>
              <button 
                onClick={() => setSelectedViewImage(null)}
                className="w-16 h-16 rounded-[1.5rem] bg-blue-600/20 backdrop-blur-xl text-white flex items-center justify-center border border-white/10 shadow-2xl hover:bg-red-500/20 transition-all"
              >
